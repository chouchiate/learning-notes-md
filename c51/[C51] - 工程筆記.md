...
#### xdata & idata
> Memory-specific pointers can be stored using
- one byte (idata, data, bdata, and pdata pointers)
...
### one byte memory pointer
```c
char data *str;        /* ptr to string in data */
int xdata *numtab;     /* ptr to int(s) in xdata */
long code *powtab;     /* ptr to long(s) in code (program) memory */
```

### 2 bytes memory pointer
```c
